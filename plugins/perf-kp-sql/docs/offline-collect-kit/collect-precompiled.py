...
CHECKS = [
    ...
    ("chk-blocked-query", "blocked_query", "db-system-view", "select pid,sessionid,substr(query,0,100) from pg_stat_activity where sessionid in(select sessionid from pg_thread_wait_status where wait_event='wait_event');", "common"),
    ("chk-blocking-query", "blocking_query", "db-system-view", "select pid,sessionid,substr(query,0,100) from pg_stat_activity where sessionid in(select block_sessionid from pg_thread_wait_status where wait_event='wait_event');", "common"),
    ...
    ("chk-thread-pool-attr", "thread_pool_attr", "db-shell", "show thread_pool_attr;", "common"),
