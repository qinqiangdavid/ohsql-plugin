...
所有 35 个 auto 命令已 inline 在 CHECKS list 里 (不解析 ndjson).

生成时间: 2026-05-31T13:04:40.155Z
...
# (check_id, name, layer, method, topology) · 35 条 auto · 直接跑
CHECKS = [
    ...
    ("chk-pg-thread-wait-status-snapshot", "pg_thread_wait_status 快照 (整合自 2 条 · 离线过滤)", "db-system-view", "SELECT node_name, db_name, thread_name, tid, sessionid, wait_status, wait_event, block_sessionid, query_id FROM pg_thread_wait_status;", "common"),
    ("chk-pg-settings-snapshot", "pg_settings 快照 (整合自 66 条 · 离线过滤)", "db-system-view", "SELECT name, setting, unit, context FROM pg_settings ORDER BY name;", "common"),
    ("chk-pgxc-stat-activity-snapshot", "pgxc_stat_activity 快照 (整合自 8 条 · 离线过滤)", "db-system-view", "SELECT coorname, pid, sessionid, datname, usename, state, waiting, query_start, query_id, query FROM pgxc_stat_activity;", "distributed-only"),
    ("chk-pgxc-thread-wait-status-snapshot", "pgxc_thread_wait_status 快照 (整合自 4 条 · 离线过滤)", "db-system-view", "SELECT node_name, db_name, thread_name, tid, sessionid, wait_status, wait_event, block_sessionid, query_id FROM pgxc_thread_wait_status;", "distributed-only"),
    ("chk-pg-stat-activity-snapshot", "pg_stat_activity 快照 (整合自 2 条 · 离线过滤)", "db-system-view", "SELECT pid, sessionid, datname, usename, state, waiting, query_start, query_id, query FROM pg_stat_activity;", "common"),
]
