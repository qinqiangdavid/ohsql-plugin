...
| `chk-slow-sql-statement-history` | statement_history 慢 SQL 明细 (全列 + 等待事件解码) | gsql -x -d postgres -c "SELECT db_name, schema_name, origin_node, user_name, application_name, client_addr, client_port, unique_query_id, debug_query_id, substr | 内核标记 is_slow_sql 的慢 SQL(阈值由 log_min_duration_statement / slow_sql_threshold 定)· 看 execution_time / 等待事件 / 锁等待定位 | 2 |
