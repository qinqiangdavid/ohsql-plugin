...
| | 数 |
|---|---:|
| **GaussDB 关联 check 合计** | **342** |
| 其中 type=metric | 273 |
| 其中 type=parameter-current-value | 69 |
...
|---|---:|
| `db-system-view` | 113 |
| `db-interactive-cmd` | 76 |
| `gaussdb-guc-param` | 69 |
| `log-grep` | 24 |
| `db-shell` | 23 |
| `os` | 17 |
| `db-internal-counter` | 15 |
| `flamegraph` | 4 |
| `log-grep / flamegraph` | 1 |

---

## collection_layer = `db-system-view` (113 check)

### type=metric (113 个)

| check_id | metric_name | collection_method | abnormal_patterns | 关联 case 数 |
|---|---|---|---|---:|
| `chk--15` | 代价估算与页面数 | 识别上面的代价估算较低，仅为18，证明pg_class中记录的页面数较少。 | "18" | 1 |
| `chk--18` | 复制槽推动速度 | select * from dbe_perf.global_replication_stat; | "主集群产生的xlog速度，明显比接收的速度要快的多" | 2 |
| `chk--22` | 磁盘读写线程 | 识别磁盘读异常SQL(根据异常IO线程号查询pg_stat_activity + pg_thread_wait_status) | "磁盘读都是Vacuum操作 / 磁盘写主要是pagewriter线程" | 1 |
| `chk--26` | 死元组数/表占用空间 | select relname, n_dead_tup, pg_relation_size(relid) sz from pg_stat_all_tables where relname='<表>'; | "死元组3600w+，表占用309GB" | 1 |
| `chk--38` | 线程等待状态 | select * from pg_thread_wait_status where query_id='149181737656737395'; | "\"根据线程等待状态，并没有出现都在等待某个DN的情况，初步排除中间结果集偏斜到了同一个DN的情况。\"" | 1 |
| `chk--39` | 表数据倾斜 | select table_skewness('ioc_dm.m_ss_index_event'); | "NULL" | 1 |
| `chk--40` | 系统表/用户表膨胀情况 | "用户可在管控面执行全库Vacuum/Vacuum Full，以定期进行空间回收" | "\"用户数据膨胀严重，磁盘空间不足，性能低。\"" | 1 |
| `chk--45` | 各节点磁盘使用率均衡性 | 登录DWS控制台。在"集群列表"页面，找到需要查看监控的集群。在指定集群所在行的"操作"列，单击"监控面板"。选择"监控 > 节点监控 > 磁盘"，查看磁盘使用率。 | "> 5% 差值" | 1 |
| `chk--47` | 表脏页率 | 查看表脏页率为99%，VACUUM FULL后性能优化到100ms左右。 | "= 99%" | 1 |
| `chk--48` | 列存表文件大小监控 | "列存表数据按列存储，一列的每60000行存储为一个CU，同一列的CU连续存储在一个文件中，当该文件大于1GB时，切换到新文件中。CU文件数据不能更改只能追加写。" | "\"列存表多次执行INSERT后，发现表膨胀。\"" | 1 |
| `chk--49` | 活跃事务列表 | SELECT txid_current_snapshot(); | "活跃事务列表中有 XID < 当前事务 XID" | 1 |
| `chk--6` | 活跃会话数 | 查看gs_asp视图查看活跃会话数在冲高时间点 | "92并发在同时运行" | 1 |
| `chk-autovacuum-settings` | autovacuum_settings | select * from pg_settings where name like '%vacuum%'; | "如果 autovacuum 被关闭或配置阈值过高，则表更新后可能长期没有触发自动分析。" | 1 |
...
| `chk-dbe-perf-statement-cpu-time` | dbe_perf.statement.cpu_time | select unique_sql_id,substr(query,1,50) as query ,n_calls,round(total_elapse_time/n_calls/1000,2) avg_time,round(total_elapse_time/1000,2) as total_time,round(c | "\"avg_time > 3ms\"" | 1 |
...
| `chk-dms` | DMS 监控 · 节点磁盘使用率 | 选择“监控 > 节点监控 > 磁盘”，单击“磁盘使用率”右侧的![](https://support.huaweicloud.com/trouble-dws/figure/zh-cn_image_0000001393399197.png)进行排序，可查看当前集群各个节点的磁盘使用率。 | ">= 70%" | 2 |
...
| `chk-last-analyze-time` | last_analyze_time | select * from PG_STAT_ALL_TABLES where relname='tablename'; | "确定系统/手动上一次 ANALYZE 是否过久。" | 1 |
| `chk-last-vacuum` | last_vacuum | 查看相关表的日志，以及pg_stat_all_tables视图，通过其中的last_vacuum字段 | "确认相关表在创建后从未vacuum" | 1 |
| `chk-lwlock-event-procarraylock` | LWLOCK_EVENT ProcArrayLock等待事件 | Wait Events Area: ‘1’ LWLOCK_EVENT ProcArrayLock 11700 (us) | "11700 (us)" | 1 |
| `chk-n-tup-hot-upd-n-tup-upd-n-dead-tup-n-live-tup` | n_tup_hot_upd / n_tup_upd / n_dead_tup / n_live_tup | 在pg_stat_all_tables中表现如下 | "HOT更新（n_tup_hot_upd），即一个元组的更新在同一block内，在所有更新（n_tup_upd）中占比很小。说明变长更新导致本来的原地更新，由于没" | 1 |
| `chk-n-tuples-fetched-n-tuples-returned` | n_tuples_fetched / n_tuples_returned | 查询dbe_perf.statement_history | "n_tuples_fetched = 2 * n_tuples_returned" | 1 |
...
| `chk-pg-index-indisusable` | pg_index_indisusable | SELECT indexrelid，indisusable FROM pg_index WHERE indrelid = '<table>'::regclass; | "false" | 1 |
...
| `chk-pg-partition` | pg_partition 各表分区数 | SELECT relname,reloptions,partcount FROM pg_class c INNER JOIN ( SELECT parentid,count(*) AS partcount FROM pg_partition GROUP BY parentid ) s ON c.oid = s.pare | "> 3000分区" | 1 |
...
| `chk-pg-stat-activity-pg-locks-sql-8-0-x` | pg_stat_activity / pg_locks 阻塞SQL（8.0.x及之前版本） | - **abnormal_patterns**: ["NULL"] | "NULL" | 2 |
...
| `chk-pg-stat-get-last-data-changed-time-2` | pg_stat_get_last_data_changed_time 最近变更的表 | SELECT table_distribution(schemaname,relname) FROM get_last_changed_table(); | "NULL" | 1 |
...
| `chk-pg-thread-wait-status-3` | pg_thread_wait_status · 线程等待状态 | SELECT * FROM pg_thread_wait_status WHERE query_id='149181737656737395'; | "`根据线程等待状态，并没有出现都在等待某个DN的情况，初步排除中间结果集偏斜到了同一个DN的情况。`" | 1 |
...
| `chk-pg-total-memory-detail` | pg_total_memory_detail | select * from pg_total_memory_detail; | "如果dynamic_used_memory较大，dynamic_used_shrctx较小，则可以确认是线程和session上内存占用较多；如果dynamic_" | 2 |
| `chk-pgxc-get-stat-all-tables-dirty-page-rate` | PGXC_GET_STAT_ALL_TABLES.dirty_page_rate | SELECT schemaname AS schema, relname AS table_name, n_live_tup AS analyze_count, pg_size_pretty(pg_table_size(relid)) as table_size, dirty_page_rate FROM PGXC_G | "> 30","dirty_page_rate > 30" | 2 |
...
| `chk-pgxc-get-table-skewness-2` | pgxc_get_table_skewness · 全库倾斜视图 | SELECT * FROM pgxc_get_table_skewness ORDER BY totalsize DESC; | "NULL" | 1 |
| `chk-pgxc-get-table-skewness-3` | PGXC_GET_TABLE_SKEWNESS 视图 | 分布差可以通过视图[PGXC_GET_TABLE_SKEWNESS]查看。 | "此处的数据分布差表示实际查询到DN上的数据量与DN平均数据量的差异。" | 1 |
| `chk-pgxc-lock-conflicts` | pgxc_lock_conflicts 锁冲突视图 | SELECT * FROM pgxc_lock_conflicts; | "NULL" | 2 |
...
| `chk-pgxc-stat-activity-state` | pgxc_stat_activity state | select state, count(*) from pgxc_stat_activity group by state; | "基本活跃会话很少，事务执行过程中状态较多，基本都在等待业务服务器向数据库发数据" | 1 |
| `chk-pgxc-stat-activity-state-2` | pgxc_stat_activity state 字段 | SELECT state, query, query_id FROM pgxc_stat_activity; | "state = 'idle in transaction'" | 1 |
...
| `chk-pgxc-thread-wait-status-wait-status` | pgxc_thread_wait_status.wait_status | Select wait_status, count(*) cnt from pgxc_thread_wait_status where wait_status not like '%cmd%' and wait_status not like '%none%' and wait_status not like '%qu | "\"通过等待视图查看作业的运行情况，发现作业总是等待部分DN，或者个别DN。\"" | 1 |
| `chk-pgxc-thread-wait-status-wait-status-wait-event` | pgxc_thread_wait_status · wait_status / wait_event | SELECT wait_status,wait_event,count(*) AS cnt FROM pgxc_thread_wait_status WHERE wait_status <> 'wait cmd' AND wait_status <> 'synchronize quit' AND wait_status | "`后台查看等待视图有大量wait wal sync和WALWriteLock状态，均为xlog同步状态。`" | 1 |
...
| `chk-pgxc-wlm-session-history-block-time-duration` | pgxc_wlm_session_history · block_time / duration | pgxc_wlm_session_history | "\"block_time较大，而duration值并无明显变化，说明用户作业受其它作业影响，在真正开始执行前进行了较长时间的排队\"" | 2 |
...
| `chk-pgxc-wlm-session-history-large-table-in-broadcast-warning` | pgxc_wlm_session_history · Large Table in Broadcast warning | "可通过pgxc_wlm_session_history或gs_wlm_session_history视图查看。" | "\"平均广播到每个DN上的数据行数 > 100,000\"" | 3 |
| `chk-pgxc-wlm-session-info-duration-block-time-query-plan-sql-has` | pgxc_wlm_session_info · duration / block_time / query_plan（按 sql_hash 比对历史） | SELECT start_time, block_time, duration, sql_hash, warning, max_peak_memory, max_spill_size, query_plan FROM pgxc_wlm_session_info were start_time > 'xxxx-xx-xx | "`找到对应的快慢语句后，对比其执行计划query_plan，发现执行计划跳变严重。`" | 1 |
...
| `chk-pgxc-wlm-session-info-streaming-stream-count` | pgxc_wlm_session_info · Streaming 算子数（stream_count） | SELECT *,(length(query_plan) - length(replace(query_plan, 'Streaming', ''))) / length('Streaming') AS stream_count FROM pgxc_wlm_session_info ORDER BY stream_co | "> 100" | 1 |
| `chk-pgxc-wlm-session-statistics-max-peak-memory-memory-skew-perc` | pgxc_wlm_session_statistics · max_peak_memory / memory_skew_percent | SELECT nodename,pid,dbname,username,application_name,min_peak_memory,max_peak_memory,average_peak_memory,memory_skew_percent,substr(query,0,50) as query FROM pg | "`根据结果中的max_peak_memory以及memory_skew_percent值，较大的值就是消耗内存较多的语句。`" | 1 |
| `chk-provolatile-proshippable` | 函数属性(provolatile/proshippable) | select proname, provolatile, proshippable from pg_proc where proname='<函数名>'; | "clock_timestamp函数的provolatile属性为‘v’不稳定的，而且proshippable为空" | 1 |
...
| `chk-table-distribution-dn-1w` | table_distribution() 各DN空间（大表个数超1W场景） | gaussdb=#SELECT schemaname,tablename,max(dnsize) AS maxsize, min(dnsize) AS minsize FROM pg_catalog.pg_class c INNER JOIN pg_catalog.pg_namespace n ON n.oid = c | "\"直接使用table_distribution()函数自定义输出，减少输出列进行计算优化\"" | 1 |
| `chk-table-distribution-dn-2` | table_distribution 各DN数据行数 | 通过table_distribution发现所有数据倾斜到了dn_6009单个DN | "`通过table_distribution发现所有数据倾斜到了dn_6009单个DN`" | 1 |
...
| `chk-thread-session-memory-context` | thread_session_memory_context | select contextname, sum(totalsize)/1024/1024 totalsize, sum(freesize)/1024/1024 freesize, count(*) sum from gs_thread_memory_context group by contextname order  | "NULL" | 1 |
| `chk-toast` | toast表空间占用 | select pg_relation_size(reltoastrelid) from pg_class where relname='<表>'; | "发现膨胀的主要再toast表上" | 1 |
...
| `chk-undo-retention-time` | undo_retention_time | show undo_retention_time; | "900s" | 1 |
| `chk-wait-cmd` | wait cmd计数 | select wait_status, wait_event, count(*) from pg_thread_wait_status group by 1,2 order by 3 desc; | "发现wait cmd计数很多，属于异常现象。" | 6 |
| `chk-wait-event-count` | wait_event_count | select wait_status,wait_event,count(*) from pg_thread_wait_status group by wait_status,wait_event order by 3 desc; | "NULL" | 1 |
| `chk-wait-read-data` | WAIT_READ_DATA | select * from dbe_perf.GLOBAL_WAIT_EVENTS where wait!=0 order by total_wait_time desc; | "查看读写data的等待事件时延等判断回放的瓶颈" | 1 |
...
## collection_layer = `db-interactive-cmd` (76 check)

### type=metric (76 个)

| check_id | metric_name | collection_method | abnormal_patterns | 关联 case 数 |
|---|---|---|---|---:|
| `chk--11` | 备机日志刷盘影响 | 将GUC参数synchronous_commit设置为local进行长稳测试。 | "将参数设置为local后，测试结果仍然存在TPS波动问题，排除备机日志刷盘对TPS波动的影响。" | 1 |
| `chk--14` | 语句执行情况与执行计划 | 通过gs_asp查看对应时间段的语句执行情况 | "大批量语句计划变为seqscan，导致消耗大量资源，引发CPU冲高。" | 1 |
| `chk--2` | 回放日志类型统计 | select * from local_xlog_redo_statics() | "关注特殊日志类型，尤其是ddl相关的xlog" | 1 |
| `chk--24` | 事务日志内容 | 使用gs_xlogdump_xid()系统函数获取被查杀事务对应的日志 | "事务生成的日志中，全都是对heap进行WAL_UHEAP_CLEAN操作，并非元组删除操作。" | 1 |
| `chk--3` | 索引页面元组分布 | 使用gs_parse_page_bypath函数解析索引表页面，分析表页面使用情况。 | "大部分叶子页面内保存的索引元组中只有10+条，而页面1中保存了200+索引元组。" | 1 |
| `chk--41` | 查询返回行数 | "检查查询语句是否返回了多余的数据信息。" | "\"对于包含50条记录的表，查询起来是很快的；但是，当表中包含的记录达到50000，查询效率将会有所下降。\"" | 1 |
| `chk--42` | 主机负载下查询单独运行时延 | "尝试在数据库没有其他查询或查询较少的时候运行查询语句，并观察运行效率。" | "\"如果效率较高，则说明可能是由于之前运行数据库系统的主机负载过大导致查询低效。\"" | 1 |
| `chk--43` | 重复执行同一查询语句的执行时间 | "重复执行相同的查询语句，如果后续执行的查询语句效率提升，则可能是由于上述原因导致。" | "\"查询效率低的一个重要原因是查询所需信息没有缓存在内存中，这可能是由于内存资源紧张，缓存信息被其他查询处理覆盖。\"" | 1 |
| `chk--5` | 索引列过滤性 | select attname, n_distinct, most_common_vals from pg_stats where tablename='<表>'; | "cus_idr过滤性不好，crt_dttm的过滤性好一些" | 1 |
| `chk-analyze-2` | ANALYZE 后的查询性能 | "使用ANALYZE命令分析数据库。" | "\"如果此命令执行后性能恢复或者有所提升，则表明AUTOVACUUM未能很好的完成它的工作，有待进一步分析。\"" | 1 |
| `chk-autoanalyze-threshold` | autoanalyze_threshold | select * from pg_stat_get_tuples_changed('table_name'::REGCLASS); select pg_autovac_status('table_name'::REGCLASS); | "确认是否达到触发autoanalyze阈值" | 1 |
...
| `chk-buffer-hit-rate-read-buffer-io` | buffer_hit_rate / read_buffer_io | select * from gs_redo_stat_info() | "90%" | 1 |
| `chk-costs` | 执行计划costs | explain (analyze, verbose, buffers) <目标SQL>; | "costs > 1000" | 12 |
...
| `chk-explain-2` | EXPLAIN · 计划与实际行数比对 | 导致执行计划选择不优 | "`统计信息不是最新的情况`" | 1 |
| `chk-explain-4` | EXPLAIN · 执行计划顺序扫描阶段耗时 | EXPLAIN` 查看多表 JOIN 执行计划 | "\"分析执行计划可知，在顺序扫描阶段耗时较多\"" | 1 |
| `chk-explain-analyze-2` | EXPLAIN ANALYZE 算子落盘标志 | "为了优化性能，可以查看SQL的执行计划，如果算子存在落盘的情况，可适当调整work_mem参数值。" | "\"如果算子存在落盘的情况\"" | 1 |
| `chk-explain-analyze-4` | EXPLAIN ANALYZE 顺序扫描耗时 | EXPLAIN | "在顺序扫描阶段耗时较多" | 7 |
| `chk-explain-analyze-agg-2` | EXPLAIN ANALYZE · Agg 算子类型及执行时间 | EXPLAIN ANALYZE` 查看聚合操作算子选择 | "\"如果大结果集选择了Sort+GroupAgg，则需要设置enable_sort=off，HashAgg耗时明显优于Sort+GroupAgg\"" | 1 |
| `chk-explain-analyze-hashjoin-dn` | EXPLAIN ANALYZE HashJoin 各 DN 执行时间范围 | EXPLAIN ANALYZE | "HashJoin的执行时间信息[2657.406,93339.924 | 3 |
| `chk-explain-analyze-join-2` | EXPLAIN ANALYZE · JOIN 算子类型及执行时间 | EXPLAIN ANALYZE` 查看两表 JOIN 的算子类型 | "\"NestLoop耗时181秒\"" | 1 |
...
| `chk-explain-nest-loop-join` | EXPLAIN 执行计划 · Nest Loop Join 耗时 | 分析该执行计划发现，扫描节点已使用Index Scan，耗时主要在最外层Nest Loop Join的Join Filter计算中，且该计算执行了字符串的加减法和不等值比较。 | "> 12s" | 2 |
...
| `chk-explain-performance` | explain performance结果 | explain performance <目标SQL>; | "没有语句不过滤分区键，未分区剪枝导致全表扫描的情况" | 1 |
| `chk-explain-performance-2` | EXPLAIN PERFORMANCE 算子耗时 | EXPLAIN PERFORMANCE | "分析发现如图红框标识的两个性能瓶颈点均为表Scan动作" | 1 |
| `chk-explain-performance-5` | EXPLAIN PERFORMANCE · 执行计划是否走向量化（列执行引擎）算子 | EXPLAIN PERFORMANCE` 查看是否有 Vector 前缀算子 | "\"经过分析发现计划走了行引擎。根本原因是：临时计划表input_acct_id_tbl和中间结果转储表row_unlogged_table使用了行存表。\"" | 1 |
...
| `chk-explain-performance-windowagg-sort` | EXPLAIN PERFORMANCE 执行计划 · WindowAgg/Sort 算子耗时 | explain performance | "\"执行计划中出现Sort和WindowAgg，第3~6步集中在一个DN上进行，使SQL非常缓慢。\"" | 2 |
...
| `chk-explain-verbose-hashjoin-2` | EXPLAIN VERBOSE · HashJoin 行数估算偏差 | set cost_param=2; explain verbose select nation, sum(amount) as sum_profit from (...) as profit group by nation order by nation | "\"导致行数估算不准确，查询性能较差\"" | 1 |
...
| `chk-explain-verbose-remotequery` | explain verbose · RemoteQuery 计划 | yshen=# set rewrite_rule='none'; SET yshen=# explain (verbose on, costs off)  select two_sum(tt.c1, tt.c2) from (select t1.c1,t2.c2 from t1,t2 where t1.c1=t2.c2 | "`该计划很慢，原因是网络传输了大量数据，然后在CN上执行HASH JOIN，不能充分利用集群资源。`" | 1 |
| `chk-explain-verbose-streaming-vs-data-node-scan` | EXPLAIN VERBOSE · 执行计划是否含 Streaming 节点 vs Data Node Scan | gaussdb=# set rewrite_rule='none'; SET gaussdb=# explain (verbose on, costs off)  select group_concat(tt.c1, tt.c2) from (select t1.c1,t2.c2 from t1,t2 where t1 | "`该计划很慢，原因是网络传输了大量数据，然后在CN上执行HASH JOIN，不能充分利用集群资源。`" | 1 |
...
| `chk-explain-verbose-subplan-2` | EXPLAIN VERBOSE · SubPlan 算子出现在目标列 | gaussdb=# set rewrite_rule='none'; SET gaussdb=# explain (verbose on, costs off) select c1,(select avg(c2) from t2 where t2.c2=t1.c2) from t1 where t1.c1<100 or | "`导致每扫描t1的一行数据，就会触发子查询的一次执行，效率低下。`" | 1 |
| `chk-explain-verbose-warning` | explain verbose WARNING · 统计信息缺失提示 | 通过explain verbose执行query分析执行计划时会提示WARNING信息，如下所示：WARNING:Statistics in some tables or columns(public.lineitem.l_receiptdate, ...) are not collected. HINT:Do ana | "`WARNING:Statistics in some tables or columns(public.lineitem.l_receiptdate, pub" | 1 |
| `chk-explain-verbose-warning-2` | EXPLAIN VERBOSE WARNING · 未收集统计信息的表/列列表 | 通过explain verbose执行query分析执行计划时会提示WARNING信息，如下所示：WARNING:Statistics in some tables or columns(public.lineitem.l_receiptdate, public.lineitem.l_commitdate, publi | "`Statistics in some tables or columns(...) are not collected.`" | 1 |
| `chk-explain-verbose-warning-3` | EXPLAIN VERBOSE 执行计划 Warning | explain verbose | "\"WARNING:Statistics in some tables or columns(public.lineitem.l_receiptdate, pub","\"执行计划中会有语句未收集统计信息的告警，并且通常E-rows估算非常小。\"" | 3 |
| `chk-explain-verbose-warning-4` | EXPLAIN VERBOSE WARNING信息 · 统计信息缺失 | "通过EXPLAIN VERBOSE执行query分析执行计划时会提示WARNING信息" | "\"WARNING:Statistics in some tables or columns(public.lineitem.l_receiptdate, pub" | 1 |
...
| `chk-in` | 执行计划in条件处理方式 | "打印语句的执行计划" | "\"执行计划中，in条件还是作为普通的过滤条件存在。这种场景下，最优的执行计划应该是将\\\"in 常量\\\"转化为join操作性能更好。\"" | 2 |
...
| `chk-nestloop-2` | 语句执行时间 / 执行计划中 NestLoop 算子 | 该问题发生在实时场景下，语句执行时间因为达到了 3600s而自动终止运行 | ">= 3600s" | 1 |
| `chk-nestloop-3` | 执行计划算子类型（NestLoop） | "首先观察SQL语句中有not in 语法；执行计划中有NestLoop" | "\"NestLoop是导致语句性能慢的主要原因。\"" | 1 |
| `chk-nestloop-4` | 执行计划算子类型（NestLoop出现） | "通过EXPLAIN VERBOSE打印语句执行计划，查看执行计划发现SQL语句中存在not in语句" | "\"NestLoop是导致语句性能慢的主要原因\"" | 1 |
...
## collection_layer = `gaussdb-guc-param` (69 check)
...
### type=parameter-current-value (69 个)
...
| `chk-advance-xlog-file-num` | advance_xlog_file_num |  |  | 1 |
| `chk-autovacuum` | autovacuum相关参数 |  |  | 1 |
| `chk-autovacuum-2` | autovacuum |  | "\"用户未开启autovacuum的同时又没有合理的自定义vacuum调度，导致表的脏数据没有及时回收，新的数据又不断插入或更新，膨胀是必然的\"" | 1 |
...
| `chk-autovacuum-ustore` | autovacuum (ustore表的自动清理) |  |  | 1 |
...
| `chk-client-encoding-server-encoding-2` | client_encoding / server_encoding |  |  | 1 |
...
| `chk-enable-codegen-2` | enable_codegen |  |  | 1 |
...
| `chk-enable-hashjoin-2` | enable_hashjoin |  | "`发现执行了Hash Join，对大表b_zyk_wbswxx（网吧上网信息）建立了Hash Table。由于该表数据量大，创建过程耗时较长。`" | 2 |
...
| `chk-log-min-duration-statement` | log_min_duration_statement |  |  | 1 |
| `chk-logintimeout` | loginTimeout |  |  | 1 |
...
| `chk-net-core-netdev-max-backlog-2` | net.core.netdev_max_backlog |  |  | 1 |
| `chk-period` | period |  | "\"普通分区表无法自动创建新分区\"" | 1 |
| `chk-psort-work-mem-2` | psort_work_mem |  | "\"如果表较大或GUC参数psort_work_mem设置较小，会导致PCK排序时产生下盘（数据库选择将临时结果暂存到磁盘），进行外部排序；一旦进行外部排序，时间消耗就会增加很多。\"","\"如果无法在内存中完成排序时，会下盘写临时文件，这时就会产生较大的影响\"" | 2 |
| `chk-qrw-inlist2join-optmode` | qrw_inlist2join_optmode |  | "\"如果优化器估算不准，可能会出现需要转化的场景没有做转化，导致性能较差。\"" | 2 |
| `chk-query-dop` | query_dop |  |  | 2 |
| `chk-recovery-max-workers-recovery-parse-workers-recovery-redo-wo-2` | recovery_max_workers/recovery_parse_workers/recovery_redo_workers |  |  | 1 |
| `chk-recovery-parse-workers` | recovery_parse_workers |  | "\"在系统长时间的运行后，备DN上会出现日志累积。当主DN故障后，数据恢复需要很长时间，数据库不可用，严重影响系统可用性。\"" | 1 |
| `chk-recovery-parse-workers-recovery-redo-workers` | recovery_parse_workers / recovery_redo_workers |  |  | 1 |
...
| `chk-sys-kernel-debug-sched-features` | /sys/kernel/debug/sched_features |  |  | 1 |
...
| `chk-thread-pool-attr-2` | thread_pool_attr |  | "\"线程池worker参数thread_pool_attr设置过小，导致业务排队。\"" | 2 |
| `chk-track-activities` | track_activities |  |  | 1 |
| `chk-ttl` | ttl |  | "\"普通分区表无法...清理过期分区\"" | 1 |
| `chk-ubtree` | ubtree页面分裂策略 |  |  | 1 |
| `chk-undo-retention-time-2` | undo_retention_time |  |  | 1 |
| `chk-vacuum-cost-delay` | vacuum_cost_delay |  |  | 1 |
| `chk-vacuum-defer-cleanup-age-2` | vacuum_defer_cleanup_age |  | "\"表示最近8000个事务产生的脏数据不进行回收\"" | 1 |
| `chk-wal-file-init-num` | wal file init num |  |  | 1 |
| `chk-walwriter-cpu-bind` | walwriter_cpu_bind |  |  | 1 |
...
## collection_layer = `log-grep` (24 check)

### type=metric (24 个)

| check_id | metric_name | collection_method | abnormal_patterns | 关联 case 数 |
|---|---|---|---|---:|
| `chk--28` | 轻量级锁排他锁等待时间日志 | 登录客户生产环境进行查询，确实找到了对应的日志打印 | "5s" | 1 |
...
| `chk-buffer-hit-rate-wait-read-data` | buffer_hit_rate / WAIT_READ_DATA | 在gs_log中检索“buffer_hit_rate”、“WAIT_READ_DATA”关键字 | "可查看历史时间段上十分钟内回放线程的缓存命中率和等待事件统计信息" | 1 |
| `chk-cn-2` | CN日志中不下推原因 | "不下推语句在pg_log中会打印不下推的原因。LOG: SQL can't be shipped, reason: ..." | "\"LOG: SQL can't be shipped, reason: With-Recursive does not contain \\\"ALL\\\" to b" | 1 |
| `chk-cn-3` | CN日志 · 不下推原因 | "不下推语句在pg_log中会打印不下推的原因，上述语句在CN的日志中会找到类似以下的日志。" | "\"不下推语句在pg_log中会打印不下推的原因\"" | 1 |
...
| `chk-freeze-page` | Freeze Page操作有效性 | 对产生的Freeze Page 类型WAL日志分析 | "nfrozen = 0" | 1 |
| `chk-gds` | GDS导入作业日志 | "检测GDS导入作业的日志，查看是否有执行失败的现象。" | "\"在导入数据失败后，占用的磁盘空间没有释放。\"" | 1 |
| `chk-gs-log` | gs_log日志清理时长 | 检查gs_log日志 | "13天" | 1 |
...
| `chk-palm-hang-detect-main` | palm_hang_detect_main等待锁日志 | gaussdb-<日期>_180856.log.gz:<日期> 某时刻.054 dn_6007_6008_6009 [unkown] [unknown] localhost 281279935201968 0[0:0#0] 0 dn_6007 0 [BACKEND] WARNING: palm_hang_detect_ | "WARNING: palm_hang_detect_main Cur time is 785847582052464, need_wait_time is 78" | 1 |
...
| `chk-print-redo-wal-count-info-print-redo-wal-time-info` | print_redo_wal_count_info/print_redo_wal_time_info | 在备机的gs_log检索“print_redo_wal”关键字 | "print_redo_wal_count_info显示的是该周期内新增回放数量（total num）最多的10种日志...print_redo_wal_time" | 1 |
| `chk-queue-usage` | queue usage | 在日志中搜索queue statistic | "有历史上各个线程队列的queue usage信息...可以从这里观察分发是否均匀" | 1 |
| `chk-redo-unlink-ddl` | redo_unlink_ddl | 在gs_log中检索“redo_unlink_ddl”关键字 | "可查看每10分钟周期内回放涉及删除文件的DDL数量" | 1 |
| `chk-subtransactions-log` | subtransactions log | 日志中可以尝试搜索关键字Transaction %lu has reached %d subtransactions! | "50000" | 1 |
| `chk-unlink-half-dead-page` | unlink_half_dead_page索引日志 | <日期> 某时刻.143 dn_6001_6002_6003 mecs mecs <IP> 281222782959760 375685[11206835某时刻7457#30802] 213523483 cn_5001 73183494132910882 [UBTREE] LOG: [unlink_half_dead_ | "valid left sibling for deletion target could not be located" | 1 |
| `chk-vacuum` | vacuum执行记录 | 通过pg_log,发现在某时刻开始，针对<库名> 中的分区sys_p22进行了vacuum。 | "此表在导数后，直到25日前，未触发vacuum 并且此表分区间存在严重的数据倾斜" | 1 |
| `chk-wal` | WAL日志产生量 | 通过计算日志里的LSN节点可知 | "10GB/h" | 1 |
| `chk-wal-2` | WAL日志类型占比 | 对WAL日志分析可知 | ">50%" | 1 |
| `chk-wal-3` | WAL日志清理模式 | 检查WAL日志 | "O(n²)" | 1 |

---

## collection_layer = `db-shell` (23 check)

### type=metric (23 个)

| check_id | metric_name | collection_method | abnormal_patterns | 关联 case 数 |
|---|---|---|---|---:|
| `chk--36` | 存储过程默认权限模式 | "存储过程默认具有SECURITYINVOKER权限。" | "\"切换test_user2执行test_user1创建的存储过程，执行报错，对表user1_tb没有权限，因为执行存储过程默认使用调用者的权限。\"" | 1 |
| `chk--44` | 写入方式 | "如果通过单条INSERT INTO语句的方式单并发写数据入库，客户端很可能会出现瓶颈" | "\"如果通过单条INSERT INTO语句的方式单并发写数据入库，客户端很可能会出现瓶颈\"" | 1 |
| `chk--46` | 表倾斜情况 | SELECT table_skewness('table name'); | "NULL" | 1 |
...
| `chk-client-encoding-server-encoding` | client encoding与server encoding配置 | show client_encoding; show server_encoding; | "业务库当前主要是encoding: GB18030-2022 （2）JDBC侧设置成非GB18030-2022和设置成GB18030-2022 （3）测试发现性" | 1 |
...
| `chk-dn-2` | 各DN数据量分布 | SELECT pg_get_tabledef('customer_t1'); | "> 10%" | 1 |
| `chk-dn-4` | 磁盘利用率各 DN 差异 | SELECT wait_status, count(*) as cnt FROM pgxc_thread_wait_status WHERE wait_status not like '%cmd%' AND wait_status not like '%none%' and wait_status not like ' | "> 5% 差值" | 1 |
| `chk-dropped-packets-count` | dropped packets count | cat /proc/net/dev | "NULL" | 1 |
...
| `chk-q-use-q-max-use-rec-cnt` | q_use / q_max_use / rec_cnt | cm_ctl query -rv或者select * from local_redo_stat(); | "主要观察q_use几个现场是否不均衡，如果不均衡的严重，那就是当前业务的特点导致cpu利用不充分" | 1 |
| `chk-recovery-max-workers-recovery-parse-workers-recovery-redo-wo` | recovery_max_workers/recovery_parse_workers/recovery_redo_workers | show recovery_max_workers; show recovery_parse_workers; show recovery_redo_workers; show shared_buffers; | "可以结合环境的机器规格和资料判断配置是否合理" | 1 |
...
| `chk-table-skewness-table-distribution-2` | table_skewness / table_distribution · 表数据倾斜率 | SELECT table_skewness('store_sales') | "\"某些DN有22831616行，其他DN都是0行，数据有严重倾斜\"" | 1 |
| `chk-thread-pool-attr` | thread_pool_attr | show thread_pool_attr; | "在现有的出口参数线程池CPU亲和设置模式下会经常触发跨NUMA" | 1 |
...
| `chk-cpu-4` | 线程CPU占用率 | top分析线程CPU占用 | "问题节点某问题DN回放相关线程CPU占用率较低（相对于对照节点某对照DN），同时某问题DN整体CPU占用率也低于某对照DN" | 1 |
| `chk-cpu-io` | CPU、IO、内存使用率 | 通过htop查看CPU、IO、内存使用情况 | "CPU还没有压满，内存充足。未发现异常" | 1 |
| `chk-cpu-io-2` | CPU、内存、IO使用率 | top -Hp <pid> | "CPU、内存、IO 使用率均很低" | 1 |
...
| `chk-io` | 磁盘IO信息 | iostat -c -x -m ${IOSTAT_INTERVAL:-1} ${IOSTAT_COUNT:-2} | "磁盘的iowait过高" | 3 |
| `chk-io-io` | 磁盘IO性能和IO调度算法 | grep -H . /sys/block/*/queue/scheduler 2>/dev/null | "无明显区别" | 1 |
| `chk-net-core-netdev-max-backlog` | net.core.netdev_max_backlog | sysctl net.core.netdev_max_backlog | "1000" | 1 |
| `chk-pidstat-iotop-i-o` | pidstat / iotop 显示线程 I/O 消耗 | "pidstat -dt -p gaussdb进程号" | "\"通常是TPLworker线程消耗的I/O读写量异常，代表用户SQL消耗I/O多\"" | 1 |
| `chk-rx-dropped-call-stack` | rx_dropped call stack | perf record -o perf_lo_drop.data -a -g -e mem:0xffff920fc37b31c8:w -- sleep 10 | "调用栈数量也基本和ifconfig lo统计dropped的差值相同，所以只需分析以上调用栈即可。...实际晚上perf监控一晚发现所有lo的丢包都来自于avc" | 1 |
...
## collection_layer = `flamegraph` (4 check)

### type=metric (4 个)

| check_id | metric_name | collection_method | abnormal_patterns | 关联 case 数 |
|---|---|---|---|---:|
| `chk--35` | 内核代码热点函数火焰图 | "如果仍然无法分析出CPU消耗原因，可以生成异常时间段内的火焰图，找到内核代码函数的瓶颈点" | "\"如果仍然无法分析出CPU消耗原因，可以生成异常时间段内的火焰图，找到内核代码函数的瓶颈点\"" | 1 |
| `chk-cpu-3` | CPU堆栈 | NULL | "这段时间的堆栈属于正常的业务流程，并无异常堆栈" | 13 |
...
| `chk-offcpu` | offcpu火焰图 | /usr/share/bcc/tools/offcputime -df -p <pid> 30 > off.stacks && flamegraph.pl off.stacks > offcpu.svg | "波动时（即下图中红框CPU利用率低的区段），由于受到日志扩页的影响，TPLworker线程都在等待日志插入","备机日志预扩同样存在阻塞" | 2 |

---

## collection_layer = `log-grep / flamegraph` (1 check)

### type=metric (1 个)

| check_id | metric_name | collection_method | abnormal_patterns | 关联 case 数 |
|---|---|---|---|---:|
| `chk-poll-interrupt` | poll_interrupt接口耗时/线程唤醒耗时 | 从日志中我们发现，CN在调用poll_interrupt接口等待DN回复消息时存在偶现时延增大到4ms的情况。 / 我们抓取了火焰图 | "4ms" | 1 |
